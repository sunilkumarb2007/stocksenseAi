...
const Watchlist = () => {
  ...
  const fetchData = async () => {
    ...
    if (!tickers || !tickers.length) { setLoading(false); return; }

    const sData = await Promise.all(
      tickers.map(async t => {
        const [stock, pred] = await Promise.all([api.getStock(t), api.getPredict(t)]);
        return stock ? { ...stock, prediction: pred } : null;
      })
    );
    setStockData(sData.filter(Boolean));
    setLoading(false);
  };
  ...
  const handleAdd = async () => {
    ...
    await api.addToWatchlist(tickerInput.trim());
    setTickerInput('');
    fetchData();
  };
    ...
    <div className="p-6 md:p-12 relative overflow-x-hidden min-h-screen bg-surface">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        ...
        <div className="relative w-full md:w-96 group">
          ...
          <input value={tickerInput} onChange={e => setTickerInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAdd()} className="w-full bg-surface-container-lowest border border-outline-variant/15 text-on-surface text-sm rounded-lg focus:ring-primary focus:border-primary block pl-12 p-3.5 transition-all outline-none" placeholder="Add Ticker (e.g., TCS, NVDA)" type="text" />
          <button onClick={handleAdd} className="absolute inset-y-1 right-1 px-3 py-1 bg-surface-container-high hover:bg-surface-bright text-on-surface rounded-md text-xs font-semibold transition-colors">Add</button>
        </div>
      </header>

      {loading ? (
        <div className="text-on-surface-variant font-body">Loading Watchlist...</div>
      ) : (
              ...
              <article key={data.ticker} className="glass-panel backdrop-blur-xl rounded-xl p-6 hover:bg-surface-container transition-all group relative overflow-hidden flex flex-col gap-4 shadow-[0_8px_30px_rgb(0,0,0,0.12)] cursor-pointer">
                <div className="flex justify-between items-start">
                  ...
                  <div className={`${isBull ? 'bg-on-secondary-container/20 text-secondary border-secondary/20' : 'bg-tertiary-container text-tertiary border-tertiary/20'} border px-3 py-1 rounded-full flex items-center gap-1.5`}>
                    ...
                    <span className="font-label text-xs font-semibold">{data.prediction?.trend || 'N/A'}</span>
                  </div>
                </div>
